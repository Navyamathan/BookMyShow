...
class CustomerActions
{
    ...
    public static Customer login() // get the parameters values when this function call (this function also returns the object BookMyShow.Customer)
    {
        System.out.println();
        System.out.println("To Login Enter The Following Details...");
        System.out.println();
        for (Customer tempAccount : BookMyShow.getCustomerArrayList())  // for loop for checking the username and password one by one in the BookMyShow.Customer arraylist
        {
            while(true)
            {
                System.out.println();
                ...
                if (accNo.equals(tempAccount.getUserName())) // checks if the userName is BookMyShow.Customer ArrayList
                {
                    System.out.print("Enter Password: ");
                    String pin = BookMyShowActions.s.nextLine(); // get the password
                    if (pin.equals(tempAccount.getPassword())) // checks if the password is BookMyShow.Customer ArrayList
                    {
                        return tempAccount; // return the BookMyShow.Customer object
                    }
    ...
    public static Customer signUp()
    {
        ...
        return temp; // return that BookMyShow.Customer object type variable
    }
    ...
    public static void availableMovies(Customer currentCustomer)
    {
        ...
        label : while(true)
        {
            ...
            {
                System.out.println("No Available BookMyShow.Movie In Your Current Location Or Date!");
            }
            ...
            while(true)
            {
                ...
                else if(choice.equalsIgnoreCase("no")) // if user choice is no then it directly call the ticket booking method
                {
                    ...
                    if(movieFound)
                    {
                        ...
                        {
                            System.out.println();
                            System.out.print("Enter The BookMyShow.Movie You Want To Book: ");
                            ...
                            {
                                System.out.println("Enter The Valid BookMyShow.Movie Name!");
                            }
                        }
                    }
                    break;
    ...
    public static LocalDate changeDateOrLocation(Customer currentCustomer)
    {
        ...
        while(true)
        {
            ...
            else if (choice == 2)
            {
                ...
                {
                    System.out.println("There is No BookMyShow.Theatre In the Location You Entered, Enter The Valid Location!");
                }
            }
            else if (choice == 3)
            {
    ...
    {
        HashMap<String, HashSet<String>> theatreShows = new HashMap<>();
        for(Movie tempMovie : selectedMovie) // pass all the ArrayList of selectedMovie one by one
        {
            Theatre theatre = tempMovie.getTheatre(); // stores the theatre object for each movie loop
            // this is true when the given theatre name is already exist so here we don't have to create a new hashSet we just have to get the existing hashSet and add the show in it
            if(theatreShows.containsKey(theatre.getTheatreName()))
            {
                String show = tempMovie.getShow().getStartingTime()+" - "+tempMovie.getShow().getEndingTime();
                theatreShows.get(theatre.getTheatreName()).add(show);
            }
            // this block will execute when the given theatre name is not already exist so here we have to create a new hashSet and add the show in it
            else
            {
                String show = tempMovie.getShow().getStartingTime()+" - "+tempMovie.getShow().getEndingTime();
                theatreShows.put(theatre.getTheatreName(),new HashSet<>());
                theatreShows.get(theatre.getTheatreName()).add(show);
            }
        }
        ...
        label : while(true)
        {
            ...
            for (String key :theatreShows.keySet()) // for loop print the available shows in the base of each theatre
            {
                if(!showFound)
                {
                    System.out.println();
                    System.out.println("...Available BookMyShow.Show...");
                    System.out.println();
                    showFound = true;
                }
                System.out.println("BookMyShow.Theatre: "+key);
                System.out.println("-----------------");
                System.out.println(theatreShows.get(key));
                System.out.println();
            }
            if (!showFound)
            {
                System.out.println("BookMyShow.Show Not Found");
            }

            while(true)
            {
                System.out.print("Enter The Starting Time You Want: ");
                ...
                {
                    System.out.println("Enter A Valid BookMyShow.Show!");
                }
            }
        }
    }
    ...
    public static int selectSeats(int screenSeatsCount, int seatsCount, String grid, HashMap<Character, ArrayList<String>> seatsGrid, Tickets tickets)
    {
        ...
        while (seatsCount > 0)
        {
           ...
           seatNumbers.add(seatNumber); // add seat number one by one in the seatNumbers array List
            ...
            if (seatsGrid.containsKey(seatRow))
            {
                ...
                ArrayList<String> selectedSeat = duplicateSeats.get(seatRow); // temp seats arraylist to store the changes
        ...
        {
            System.out.println();
            System.out.println("Do You Want Book The BookMyShow.Tickets?");
            System.out.print("(yes/no): ");
    ...
    public static void viewTicket(Customer currentCustomer)
    {
        ...
        for(Tickets tempTicket : currentCustomer.getTicketList()) // loop to get the currentCustomers ticket ArrayList one by one
        {
            ...
            System.out.println("BookMyShow.Screen: "+tempTicket.getScreenName());
            System.out.println("BookMyShow.Show Time: "+tempTicket.getStartingTime());
